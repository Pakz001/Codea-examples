...
function turret:init(x,y)
    ...
    self.targetangle = 0
    self.turnspeed = 0.2
    self.enemy = 1 -- 1 = player , 2 = agents
    self.frame = 1 -- our animation frame 1 is up clockwise to 8. 2 = right up.
end
turrets = {}
...
particle = class()
function particle:init(x,y,angle,timeout) -- angle in degrees
    self.x = x
    self.y = y
    self. active = true
    self.incx = math.cos(math.rad(angle))
    self.incy = math.sin(math.rad(angle))
...
function explosion:init(x,y)
    ...
    self.y = y
    self.r = 0
    self.maxr = math.random(16,96)
    self.state = 1
    self.inc = 0.02
    self.incspd = (math.random()/10)+0.02
end
explosions = {}
...
-- breaking fx and system for breaking(mining) blocks.
...
function pickup:init(x,y,num)
    ...
    self.tilenum = num
    self.remove = false
    self.timeout = (60*60)*2--remove after 2 minutes
end
pickups = {}
...
function setup()
    ...
    for i=1,27 do
        table.insert(playerinventory,inventory())
    end
    --insertinventory(pickupsilverore,1000)
    --insertinventory(pickupgoldore,64)
    ...
    turretframe = {}
    for i=0,7 do
        table.insert(turretframe,mytiles:copy(272+(i*16),416,16,16))
    end
    
    uiinvlefttopim = 7
    uiinvcentertopim = 8
    uiinvrighttopim = 9
    uiinvleftcenterim = 10
    uiinvcenterim = 11
    uiinvrightcenterim = 12
    -- set up the user interface inventory tiles
    uiinvlefttop = myuitiles:copy(113,417,16,16)
    --uiinvleft = myuitiles:copy(,16,16)
    uiinvcentertop = myuitiles:copy(129,417,16,16)
    uiinvrighttop = myuitiles:copy(145,417,17,16)
    uiinvleftcenter = myuitiles:copy(113,401,16,16)
    uiinvcenter = myuitiles:copy(129,401,16,16)
    --uiinvrighttop = myuitiles:copy(,16,16)
    uiinvrightcenter = myuitiles:copy(145,401,16,16)
    ...
    inventoryicon = mytiles:copy(273,465,16,16)
    ...
    insertpickup(400,300,pickupgoldore)
    insertpickup(430,300,pickupsilverore)
    insertpickup(460,300,pickupironore)
    insertpickup(490,300,pickupstoneore)
    
    for i=1,10 do
        iniexplosion(WIDTH/2+math.random(-100,100),HEIGHT/2+math.random(-100,100))
    end
    iniparticles(WIDTH/2+math.random(-100,100),HEIGHT/2+math.random(-100,100),math.random(100,250))
    
    for i=0,10 do
    table.insert(agents,agent(WIDTH/2,HEIGHT/2))
    end
    
    initurret(WIDTH/2,HEIGHT/2)
end
...
function draw()
    ...
    updateposition()
    drawstars()
    lightplayer((WIDTH/2+position.x)/tilewidth,(HEIGHT/2+position.y)/tileheight)
    drawmap()
    if playeritemselected==iteminventory then drawinventory() end
    shipcontrols()
...
-- add a turret
function initurret(x,y)
    table.insert(turrets,turret(x,y))
end
...
function updateturrets()
    ...
    for i=1,#turrets do
        if turrets[i].active==true then
            turrets[i].x=turrets[i].x-math.cos(currentangle)*shipspeed 
            turrets[i].y=turrets[i].y-math.sin(currentangle)*shipspeed
            
            if vec2((position.x+WIDTH/2),position.y+HEIGHT/2):dist(vec2(turrets[i].x,turrets[i].y))<1000 and math.random()<.05 and turrets[i].enemy==1 then -- if we need to target the player
                
                a = math.atan2(turrets[i].y-(position.y+HEIGHT/2),turrets[i].x-(position.x+WIDTH/2))
                b = math.atan2((position.y+HEIGHT/2)-turrets[i].y,(position.x+WIDTH/2)-turrets[i].x)
                turrets[i].angle = b
                -- turn the angle into the turret frame.
                turrets[i].frame=math.floor((8/360*((360)-(math.deg(a)+180)))+3)
                if turrets[i].frame<1 then turrets[i].frame=1 end
                if turrets[i].frame>8 then turrets[i].frame=turrets[i].frame-8 end
                --if math.random()<.1 then print ((8/360*((360)-(math.deg(a)+180)))+1) end
                --if math.random()<.1 then print (turrets[i].frame) end
                if math.random()<0.1 then
                    table.insert(laserbolts,laserbolt(turrets[i].x+math.cos(b)*16,turrets[i].y+math.sin(b)*16,b))
                end
            end
...
function drawturrets()
    ...
    for i=1,#turrets do
        if turrets[i].active==true then
            ...
            --sprite(im,0,0)
            scale(4,4)
            sprite(turretframe[turrets[i].frame],0,0)
            popMatrix()
        end
    end
 end   
function updateagents()
    ...
    for i=1,#agents do
        if agents[i].active==true then
            agents[i].x=agents[i].x-math.cos(currentangle)*shipspeed 
            agents[i].y=agents[i].y-math.sin(currentangle)*shipspeed
            agents[i].targetx=agents[i].targetx-math.cos(currentangle)*shipspeed 
            agents[i].targety=agents[i].targety-math.sin(currentangle)*shipspeed
            agents[i].startx=agents[i].startx-math.cos(currentangle)*shipspeed 
            agents[i].starty=agents[i].starty-math.sin(currentangle)*shipspeed
            if agents[i].state==stateroam then
                d = vec2(agents[i].x,agents[i].y):dist(vec2(agents[i].targetx,agents[i].targety))
                if d<20 then
                    agents[i].targetx = agents[i].startx + math.random(-1550,1450)
                    agents[i].targety = agents[i].starty + math.random(-1450,1540)
                    --print(math.random())
                    --agents[i].targetx = position.x
                    --agents[i].targety = position.y
                    agents[i].targetangle = math.atan2(agents[i].targety-agents[i].y,agents[i].targetx-agents[i].x)
                end
                if orientation(agents[i].x,agents[i].y,agents[i].x+math.cos(agents[i].angle)*500,agents[i].y+math.sin(agents[i].angle)*500,agents[i].targetx,agents[i].targety) == -1 then
                    agents[i].angle = agents[i].angle - .01
                else
                    agents[i].angle = agents[i].angle + .01
                end
                agents[i].x = agents[i].x + math.cos(agents[i].angle)
                agents[i].y = agents[i].y + math.sin(agents[i].angle)
            end 
        end
    end
end
...
function drawagents()
    ...
    for i=1,#agents do
        if agents[i].active==true then
            pushMatrix()
            translate(agents[i].x,agents[i].y)
            scale(gamescale,gamescale)
            rotate(math.deg(agents[i].angle)-90)
            --sprite(im,0,0)
            scale(4,4)
            sprite(ship,0,0)
            popMatrix()
...
function iniparticles(x,y,timeout)
    for i=1,360,25 do
        table.insert(particles,particle(x,y,i,timeout))
    end
end
...
function updateparticles()
    ...
    for i=1,#particles do
        if particles[i].active==true then
            particles[i].x=particles[i].x-math.cos(currentangle)*shipspeed 
            particles[i].y=particles[i].y-math.sin(currentangle)*shipspeed
            particles[i].time = particles[i].time + 1
            if particles[i].time > particles[i].timeout then particles[i].active=false end
            particles[i].x = particles[i].x + particles[i].incx
            particles[i].y = particles[i].y + particles[i].incy
        end
    end
    again=true
    while again==true do
...
function iniexplosion(x,y)
    if explosions==nil then
        table.insert(explosions,explosion(x,y))
        iniparticles(x,y,math.random(50,250))
        return
    end
...
function updateexplosions()
    if explosions==nil then return end
    for i=1,#explosions do
        if explosions[i].active==true then
            explosions[i].x=explosions[i].x-math.cos(currentangle)*shipspeed 
            explosions[i].y=explosions[i].y-math.sin(currentangle)*shipspeed
            if explosions[i].state==1 then
                explosions[i].r = explosions[i].r + explosions[i].inc
                explosions[i].inc = explosions[i].inc + explosions[i].incspd
                if explosions[i].r > explosions[i].maxr then explosions[i].state=2 end     
            end
            if explosions[i].state==2 then
                explosions[i].r = explosions[i].r - explosions[i].inc
                explosions[i].inc = explosions[i].inc - explosions[i].incspd
                if explosions[i].r < 0 then explosions[i].active=false end
                end
            end
    end
end
...
function drawexplosions()
    ...
    for i=1,#explosions do
        if explosions[i].active==true then       
            ellipse(explosions[i].x,explosions[i].y,explosions[i].r,explosions[i].r)
        end
    end
end

function insertinventory(item,count)
    -- find inside the inventory slots if there is the given
    -- item and if there is space to store the count.
    for i=1,#playerinventory do
        if playerinventory[i].item == item and playerinventory[i].count < 64 then
            playerinventory[i].count = playerinventory[i].count + count
            if playerinventory[i].count <= 64 then return end
            count = playerinventory[i].count-64
            playerinventory[i].count = 64
            
        end 
    end
    -- if we have count of item left then try to create
    -- a new item on a new slot.
    for i=1,#playerinventory do
        if count>0 and playerinventory[i].item == 0 then
            playerinventory[i].item = item
    ...
    do return count end
    --for i=1,#playerinventory do 
    --    playerinventory[i].item = math.random(1,4)
    --    playerinventory[i].count = math.random(0,64)
    --    if playerinventory[i].count==0 then playerinventory[i].item=0 end
    --end
    
end

function drawinventory()
    iwidth = math.floor(10*48)
    ...
    cnt=1
    for y=topy,(topy-(48*4)),-47 do
        tint(255)
        drawuiblock(left,y,uiinvlefttopim)
        for x=left+47,(left+iwidth)-48,48 do
            if y==topy then
                drawuiblock(x,y,uiinvcentertopim)
            else
                --tint(0)
                drawuiblock(x,y,uiinvcenterim)
                if cnt<28 then
                    fill(0)
                    stroke(255)
                    strokeWidth(1)
                    ...
                    if playerinventory[cnt].count>0 then
                        tint(255)
                        pushMatrix()
                        translate(x,y)
                        scale(3,3)
                        if playerinventory[cnt].item == pickupsilverore then
                            sprite(silveroredrop,8,8)
                        elseif playerinventory[cnt].item == pickupgoldore then
                            sprite(goldoredrop,8,8)
                        elseif playerinventory[cnt].item == pickupironore then
                            sprite(ironoredrop,8,8)
                        elseif playerinventory[cnt].item == pickupstoneore then
                            sprite(stoneoredrop,8,8)
                        end
                        popMatrix()
                        fill(255)
                        fontSize(16)
                        text(playerinventory[cnt].count,x+38,y+12)
                    end
                    cnt=cnt+1
        ...
        tint(255)
        drawuiblock((left+iwidth),y,uiinvrighttopim)
    end
...
function playergetpickups()
    ...
    for i=0,#pickups do      
        if pickups[i] ~= nil and pickups[i].active then
            ...
            if d<16 then
                ...
                insertinventory(pickups[i].tilenum,1)
            end
        end        
    end
...
function insertpickup(x,y,num)
    ...
    for i=0,#pickups do
        if pickups[i]~=nil and pickups[i].active==false then
            ...
            pickups[i].x = x
            pickups[i].y = y
            pickups[i].timeout = (60*60)*2
            pickups[i].tilenum = num
            
            pickups[i].active=true
            pickups[i].remove=false
            return
        end
    end
    table.insert(pickups,pickup(x,y,num))
...
function updatepickups()
    ...
    for i=0,#pickups do      
        if pickups[i] ~= nil and pickups[i].active==true then
            ...
            pickups[i].timeout = pickups[i].timeout - 1
            if pickups[i].timeout<0 and vec2(WIDTH/2,HEIGHT/2):dist(vec2(pickups[i].x,pickups[u].y))<1000 then
                pickups[i].active=false
            else
                pickups[i].timeout=(60*60)*2
            end
...
function drawuiblock(x,y,block)
    ...
    if block == uiinvlefttopim then
        sprite(uiinvlefttop,8,8)
    end
    if block == uiinvcentertopim then
        sprite(uiinvcentertop,8,8)
    end
    if block == uiinvrighttopim then       
        sprite(uiinvrighttop,8,8)
    end
    if block == uiinvleftcenterim then       
        sprite(uiinvleftcenter,8,8)
    end
    if block == uiinvcenterim then       
        sprite(uiinvcenter,8,8)
    end
    if block == uiinvrightcenterim then       
        sprite(uiinvrightcenter,8,8)
    end
    popMatrix()
end
    ...
    if CurrentTouch.state == BEGAN then
        ...
        elseif doubletap==2 and doubletaptime>30 then
            ...
            if laserbolts==nil then
                ...
                chosen=false
                for i=1,#laserbolts do
                    if laserbolts[i]==nil then
                        laserbolts[i]= laserbolt(WIDTH/2,HEIGHT/2,currentangle)
                        chosen=true
                        break
                    end
                end
...
function inilaserbolt(x,y,angle)
    if laserbolts==nil then
        table.insert(laserbolts,laserbolt(x,y,angle))
        do return end
    end
end
...
function updatelaserbolts()
    ...
    for i=1,#laserbolts do
        if laserbolts[i].active==true then
            -- update the laserbolts with the player map position.
            ...
            for j = 1,#agents do
                if agents[j].active==true and laserbolts[i].active==true then
                    d = vec2(agents[j].x,agents[j].y):dist(vec2(laserbolts[i].x,laserbolts[i].y))
                    if d<16 then
                        iniexplosion(laserbolts[i].x,laserbolts[i].y)
                        laserbolts[i].active = false
    ...
    while rem==true do
        rem=false
        for i=1,#laserbolts do
            if laserbolts[i].active==false then
                table.remove(laserbolts,i)
                rem=true
                break
            end
        end
    ...
    for y=-2,2 do
        for x = -2,2 do
            ...
            if rectsoverlap(CurrentTouch.x,CurrentTouch.y,1,1,x1+offx+(tilewidth/2),y1+offy+(tileheight/2),tilewidth,tileheight) then
                ...
                map[tilex][tiley]=math.random(3,7)
...
function lightplayer(x,y)
    x = math.floor(x)
    y = math.floor(y)
    
    if lightmap[x]==nil or lightmap[x][y]==nil then return end
    if lightmap[x-20]==nil or lightmap[x+20]==nil or lightmap[x-20][y]== nil or lightmap[ x+20][y]==nil then return end
    if lightmap[x][y-20]==nil or lightmap[x][y+20]==nil then return end 
    for ya=y-20,y+20 do
        for xa=x-20,x+20 do
            lightmap[xa][ya]=0
        end
    ...
    while #openlist>0 do
        ...
        if lightmap[x][y]>10 and vec2(x,y):dist(orig)<12 then
            for i = 1,#switch do
                ...
                --if pos.x>=0 and pos.y>=0 and pos.x<mapw and pos.y<mh then
                    if lightmap[pos.x][pos.y]==0 then
                        if map[x][y]==0 or map[x][y]==2 or map[x][y]==nil then
                            lightmap[pos.x][pos.y]=lightmap[x][y]/100*90
                        else
                            lightmap[pos.x][pos.y]=lightmap[x][y]/100*70
                        end
                        if map[pos.x][pos.y]~=nil and map[pos.x][pos.y]>0  then
                            --lightmap[pos.x][pos.y]=lightmap[pos.x][pos.y]/4
                        end
    ...
    for x=0,20,1 do
        for y=0,20,1 do
            if map[x+tilex]==nil==false and map[x+tilex][y+tiley]==nil==false then
                ...
                scale(4,4)
                tint(255)
                if lightmap[x+tilex]~=nil and lightmap[x+tilex][y+tiley]~=nil then
                    tint(lightmap[x+tilex][y+tiley])
                    --if math.random()<.005 then print (lightmap[x+tilex][y+tiley]) end
                end
                if map[x+tilex][y+tiley]==1 then
                    sprite(block1,8,8)
...
function updatebuildicon()
    ...
    -- inventory button
    if rectsoverlap(CurrentTouch.x,CurrentTouch.y,1,1,left+tilewidth*3,50,hor,50) == true then
        if CurrentTouch.state == BEGAN then
            buildgridactive=false
            if playeritemselected~=iteminventory then
                playeritemselected = iteminventory
            else
                playeritemselected=-1
            end
            buildgriddelay=40
...
function drawbuildicon()
    ...
    fill(0)
    stroke(255, 0, 66)
    if playeritemselected==iteminventory then stroke(255) end
    rect(left+tilewidth*3,50,hor,50)
    tint(255)
    pushMatrix()
    translate(left+tilewidth*3,50)
    scale(3,3)
    sprite(inventoryicon,8,8)
    popMatrix()
