...
function draw()
    ...
    if dis<40 then
        dest.x = pos.x + math.random(-1000,1000)
        dest.y = pos.y + math.random(-1000,1000)
    end
    --des.x = des.x/dis
    --des.y = des.y/dis
...
function scrollplanes()
    for i = 1,#planes do
        planes[i].pos.x=planes[i].pos.x-vel.x
        planes[i].pos.y=planes[i].pos.y-vel.y
        planes[i].dest.x=planes[i].dest.x-vel.x
        planes[i].dest.y=planes[i].dest.y-vel.y
    end
end
...
function scrollbullets()
    for i =1,#bullets do
        bullets[i].pos.x = bullets[i].pos.x - vel.x
        bullets[i].pos.y = bullets[i].pos.y - vel.y
    end
end
