...
function draw()
    ...
    drawmap()
    --drawpathmap()
    if math.random()<1 then updatephas() end
    drawphas()
end
...
function updatephas()
    for i=1,#phas do
        ...
        for j=1,#spots do
            ...
            if b<lowest then
                lowest = b
                phas[i].pos.x = spots[j].x
                phas[i].pos.y = spots[j].y
...
function drawphas()
    ...
    stroke(255, 8, 0)
    for i=1,#phas do
        rect(phas[i].pos.x*tilewidth-4,phas[i].pos.y*tileheight-4,tilewidth+8,tileheight+8)
    end
end
