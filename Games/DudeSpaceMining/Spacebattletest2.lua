...
function draw()
    ...
    if dis<40 then
        dest.x = math.random(50,WIDTH-50)
        dest.y = math.random(50,HEIGHT-50)
    end
    --des.x = des.x/dis
    --des.y = des.y/dis
...
function updateplanes()
    for i = 1,#planes do
        ...
        if math.random()<.1 then
            ...
            if linecirclecollide(planes[i].pos.x,planes[i].pos.y,planes[i].pos.x+planes[i].vel.x*460,planes[i].pos.y+planes[i].vel.y*460,pos.x,pos.y,32) then
                vx=planes[i].vel.x
                vy=planes[i].vel.y
